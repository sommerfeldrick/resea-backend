/**
 * medRxiv API Service
 * https://api.biorxiv.org/ (same API as bioRxiv)
 *
 * Medical/health sciences preprint repository with:
 * - 30K+ preprints
 * - All open access
 * - No API key required
...
export class MedRxivService extends BaseAPIService {
  constructor() {
    super(
      'medRxiv',
  ...
  async search(
    ...
    try {
      ...
      const response = await this.makeRequest<{ collection: MedRxivMessage[] }>({
        method: 'GET',
        url: `/details/medrxiv/${this.formatDate(startDate)}/${this.formatDate(endDate)}`,
      });
  ...
  private parseMessage(message: MedRxivMessage): AcademicArticle | null {
    try {
      ...
      // Build URLs
      const pdfUrl = `https://www.medrxiv.org/content/${message.doi}.full.pdf`;
      const url = `https://www.medrxiv.org/content/${message.doi}`;
